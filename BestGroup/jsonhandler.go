...
func InitJSON(jsonI string, Stype bool) error {
...
func SaveJSON(jsonI string, data1 AutoGenerated, data2 Users) error {
	muNodes.Lock()
...
func FormatNodes(format string) (string, error) {
	muNodes.RLock()
	defer muNodes.RUnlock()
...
func GetNodes() string {
	...
	sort.Slice(Nodes.Nodes, func(i, j int) bool {
		return Nodes.Nodes[i].ID > Nodes.Nodes[j].ID
	})

	sort.Slice(Nodes.Decrypted, func(i, j int) bool {
		return Nodes.Decrypted[i].ID > Nodes.Decrypted[j].ID
	})
	fChild := true
	for _, node := range Nodes.Nodes {
		//check if Nodes.Decrypted[count] is not nil
		str += "<ul class=\"list-group list-group-horizontal\" style=\"justify-content: center;\"> <li class=\"list-group-item\""
		if fChild {
			str += " id=\"fChild\""
			fChild = false
		}
		...
		if len(Nodes.Decrypted) > count && Nodes.Decrypted[count].ID == node.ID {
			str += "<span class=\"tooltiptext\">" + strconv.Itoa(Nodes.Decrypted[count].LEDBrightness) + " " + strconv.Itoa(Nodes.Decrypted[count].LEDUsage) + " " + strconv.Itoa(Nodes.Decrypted[count].DeadLEDs) + " " + strconv.FormatBool(Nodes.Decrypted[count].IsPowered) + " " + strconv.FormatBool(Nodes.Decrypted[count].ZMCTError) + " " + strconv.FormatBool(Nodes.Decrypted[count].XY_MOSError) + " " + strconv.FormatBool(Nodes.Decrypted[count].ACSError) + " " + strconv.FormatBool(Nodes.Decrypted[count].PhotoresistorError) + " " + strconv.FormatBool(Nodes.Decrypted[count].RequestBackupData) + "</span>"
			count++
		}
		str += "</li></ul>"
	}
